...
export type RunMode = "highway" | "speed_round";
...
const DISPLAY_NAME_PATTERN = /^[\w .'-]{2,24}$/;
...
const RUN_MODES = new Set(["highway", "speed_round"]);
...
export function parseRoundWritePayload(value: unknown): RoundWritePayload | null {
  ...
  if (
    ...
    !isPatternString(value.displayName, DISPLAY_NAME_PATTERN) ||
    ...
    !isBoundedInteger(value.durationMs, 0, 180_000)
  ) {
    return null;
  }
  ...
  return {
    ...
    runMode: value.runMode as RunMode,
...
function parseMasteryDelta(value: unknown): RoundMasteryDelta | null {
  if (
    ...
    !isBoundedInteger(value.attempts, 0, 1_000) ||
